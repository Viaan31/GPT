...
const ARCHIVE_PAGE_1 = "/2023";
const ARCHIVE_PAGE_2 = "/2022";
const ARCHIVE_CLASS = ".has-featured-img";
...
const getLinks = async (page: string,className: string) => {
  ...
  const filteredLinks = [...new Set(links.filter((link) => link.endsWith("/")))];
...
const getPost = async (url: string, type: "post" | "mini") => {
  ...
  console.log(date)
  const text = $(".entrytext").text().trim();
